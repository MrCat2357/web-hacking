...
## 6.3 Comandos importantes

Para descobrir a versão do mariaDB:

`select @@version;`

```mysql
MariaDB [desec]> select @@version;
+------------------+
| @@version        |
+------------------+
| 11.4.5-MariaDB-1 |
...
O terminal do BD funciona também como calculadora:

`select 128+64;` 

```mysql
MariaDB [desec]> select 128+64;
+--------+
| 128+64 |
+--------+
|    192 |
...
Podemos também usá-lo para carregar arquivos:

`select load_file('/var/www/html/index.html'); `

```mysql
MariaDB [desec]> select load_file('/var/www/html/index.php');
+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Com isso poderemos, durante um ataque de SQLi, ler arquivos armazenados em um servidor.

Um outro comando interessante que podemos aprender é o do `sleep`, que faz o terminal esperar o tempo setado:

`select sleep(2)`;

```mysql
MariaDB [desec]> select sleep(3);
+----------+
| sleep(3) |
+----------+
|        0 |
...
Isso é importante pelo fato de algumas vezes estarmos mapeando às cegas certos bancos de dados e  sleep pode ser uma ferramenta útil para ver a resposta de que o banco de dados está aceitando nossos comandos.

Outro comando importante é o `select char()` usado para traduzir elementos da tabela [ASCII](https://www.ascii-code.com/pt). 

`select char(65);`

```mysql
MariaDB [desec]> select char(65);
+----------+
| char(65) |
+----------+
| A        |
...
Em alguns casos o `char` pode ser usado para bypass

Outras ferramentas úteis para mapear a superfície de ataque de um banco de dados são a `length` e a `substring`  

Com a `length` podemos descobrir o tamanho das string em análise:

`select length("desec");`

```mysql
MariaDB [desec]> select length("desec");
+-----------------+
| length("desec") |
+-----------------+
|               5 |
...
Para ir descobrindo a palavra ou nome, podemos usar o `substring`  e ir incrementando cada string em uma espécie de brute force para descobrir nome de tabelas ou bancos de dados.

`select substring("desec",1,2)`

```mysql
MariaDB [desec]> select substring("desec",1,1);
+------------------------+
| substring("desec",1,1) |
+------------------------+
| d                      |
...
MariaDB [desec]> select substring("desec",1,2);
+------------------------+
| substring("desec",1,2) |
+------------------------+
| de                     |
...
MariaDB [desec]> select substring("desec",2,1);
+------------------------+
| substring("desec",2,1) |
+------------------------+
| e                      |
...
MariaDB [desec]> select substring("desec",1,3);
+------------------------+
| substring("desec",1,3) |
+------------------------+
| des                    |
...
Por fim, temos o `drop` que serve para excluir itens. Iremos criar uma database chamada de `teste` apenas para excluí-la usando o `drop`:
...
```mysql
MariaDB [desec]> create database teste;
Query OK, 1 row affected (0.005 sec)

MariaDB [desec]> show databases;
+--------------------+
| Database           |
+--------------------+
| desec              |
...
MariaDB [desec]> drop database teste;
